...
from config import CACHE_DIR, CALL_DELAY_SECONDS, GATED_MUTE_CONFIDENCE, PROMPT_VERSION, ROUTING_CACHE
...
def decide(msg: IncomingMessage, builder: ContextBuilder, llm_router) -> RoutingDecision:
...
def run(
    ...
    for i, msg in enumerate(pending, 1):
        try:
            decision = decide(msg, builder, llm_router)
